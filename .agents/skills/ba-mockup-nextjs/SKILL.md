...
description: >-
  ...
  รองรับธีมภายนอก (Figma/Google Stitch/Design.md) และธีม Deves (ระบบ EDNS) เป็นค่าเริ่มต้น
  ใช้เมื่อผู้ใช้ต้องการสร้าง mockup หน้าจอ UI ระบบเว็บ
---
...
2. **ถ้าผู้ใช้ให้ลิงก์ Figma**: ดึงข้อมูล design tokens จริงจากไฟล์ Figma (สี, ฟอนต์, spacing, component) แล้วแปลงเป็น Tailwind config ไม่ใช่เดาจากภาพหรือชื่อไฟล์

3. **ถ้าผู้ใช้แนบไฟล์ Design.md หรือเอกสารสรุปดีไซน์**: อ่านไฟล์นั้นแล้วดึง design tokens (สี, ฟอนต์, radius, spacing, component pattern) ออกมาให้ครบ ถ้าข้อมูลไม่ครบ (เช่น ไม่มีสี danger/warning) ให้ถามผู้ใช้เพิ่ม ห้ามเดาเอง

4. **ถ้าผู้ใช้แนบโค้ดที่ export จาก Figma/Stitch หรือโค้ดต้นฉบับอื่นๆ** (เช่น React/HTML/CSS component, JSON design tokens, หรือแม้แต่โปรเจกต์เว็บที่มีอยู่แล้ว): อ่านโค้ดนั้นแล้วแกะ design tokens และ component pattern ออกมาจากของจริง (ดูสี, font, spacing, border-radius, shadow จาก CSS/className จริงในไฟล์) ไม่ใช่เดาจากชื่อไฟล์หรือชื่อโปรเจกต์
...
- **`Card`** — กล่องพื้นขาว `rounded-lg` (หรือ `rounded-xl`) shadow เบาๆ (`shadow-sm`) padding `1.25rem` — ใช้ครอบทุก section
- **`SummaryCard`** — เหมือน Card แต่มี `border-left` หนา 4px สีตามความหมาย (รับ prop `color`), ตัวเลขใหญ่ตัวหนา (`text-2xl font-bold`), label เล็กสีเทา (`text-muted`), icon อยู่ขวาบนในกรอบวงกลม/สี่เหลี่ยมโปร่งใสสีเดียวกับ border
...
2. **ทำขั้นตอน "การเลือกแหล่งธีม" ก่อนเสมอ** ในทุกงานใหม่
3. ก่อน scaffold โปรเจกต์ใหม่ครั้งแรก ต้องยืนยัน scope หน้าที่จะทำ mockup กับผู้ใช้ก่อนเสมอ
4. ย้ำเสมอว่างานนี้คือ **"mockup หน้าบ้านเท่านั้น"** — ห้ามสร้าง backend/database/API จริง/auth จริง
5. ให้ความสำคัญกับ **responsive** และ **accessibility พื้นฐาน**
6. **แจ้งผู้ใช้ก่อนรันคำสั่ง shell ที่ scaffold โปรเจกต์หรือติดตั้ง dependency ทุกครั้ง**
7. เมื่อสร้าง/แก้ไขหน้าใหม่แต่ละหน้า ให้สรุปสั้นๆ ว่าไฟล์ไหนถูกสร้าง/แก้ และชี้จุดที่เป็น mock/placeholder
8. หากผู้ใช้แนบไฟล์หรือโฟลเดอร์โค้ดต้นฉบับใดๆ มาให้ดูเป็น reference ให้ **อ่านอย่างเดียว ห้ามแก้ไข/ลบไฟล์เหล่านั้นเด็ดขาด**
