...
> เวอร์ชัน SRS อ้างอิง: 3.3.0 (06 กันยายน 2569)
...
| **Version** | **3.3.0 (ปรับปรุงระบบ EDR Memo: แยกสิทธิ์ Admin ฝ่าย vs SuperAdmin, AD Auto-Provisioning, ยกเลิก DocType, เพิ่ม Audit Log ยกเลิก/ลิงก์, และปุ่ม Copy เลข)** |
| **Prepared by** | Business Analyst (BA) ประจำโครงการ — บริษัท เทเวศประกันภัย จำกัด (มหาชน) |
| **Created Date** | 31 สิงหาคม 2569 (ปรับปรุงล่าสุด: 06 กันยายน 2569) |
...
| V3.2.0 | 05 ก.ย. 2569 | แยกบทบาทผู้ใช้ สิทธิ์การใช้งาน และเนื้อหารายหมวดออกจากกันอย่างเด็ดขาด (Decoupled by System) พร้อมรูปภาพประกอบ Retina 36 ภาพ | BA |
| **V3.3.0** | **06 ก.ย. 2569** | **ปรับปรุงข้อกำหนดระบบ EDR Memo (ระบบขอเลขบันทึกภายใน) เพื่อความคล่องตัวและรัดกุมสูงสุด:**<br/>1) **แยกสิทธิ์ Admin ฝ่าย vs SuperAdmin:** เพิ่มบทบาท `Admin ฝ่าย (Department Admin)` เห็นและจัดการงานของทุกคนในฝ่ายตนเอง 100% แต่ **ไม่มีสิทธิ์เข้าถึงเมนูตั้งค่าระบบ (Settings/Master Data)** ส่วน `SuperAdmin` มีสิทธิ์สูงสุดดูแลทั้งองค์กรและ Master Data ทั้งหมด<br/>2) **Active Directory Auto-Provisioning:** ผู้ใช้งานทั่วไปล็อกอินผ่าน AD แล้วระบบสร้างบัญชีกำหนดสิทธิ์ผู้ใช้ทั่วไปให้อัตโนมัติทันที ไม่ต้องให้ Admin เซ็ตสิทธิ์ล่วงหน้า<br/>3) **ยกเลิกประเภทเอกสารบันทึกภายในทั้งหมด:** ตัดเรื่องประเภทเอกสาร (DocTypes), Dropdown, Quick Add และ Master ประเภทเอกสารออกทั้งหมดเพื่อความสะดวกรวดเร็ว<br/>4) **เพิ่มการบันทึก Audit Log:** บันทึกประวัติการกดยกเลิกเอกสาร (พร้อมบังคับระบุเหตุผล) และการเพิ่ม/แก้ไข/ลบ ลิงก์เอกสารอ้างอิง<br/>5) **เพิ่มปุ่มคัดลอกเลขเอกสาร (One-Click Copy):** เพิ่มปุ่ม Copy เลขเอกสารลง Clipboard ในหน้าต่างสำเร็จ, หน้ารายละเอียดคำขอ และตารางรายการ | BA |
...
|---|---|---|---|
| **BR-EDR-001** | **EDR Memo** | ผู้ใช้สามารถเลือกโครงสร้างหน่วยงาน 3 ระดับ (สายงาน $\rightarrow$ ฝ่าย $\rightarrow$ หน่วยงาน/ทีม) และระบบแสดง Real-time Preview เลขที่จะได้รับตาม Master โดยไม่ต้องระบุประเภทเอกสาร | High |
...
| **BR-EDR-003** | **EDR Memo** | ระบบรองรับการเข้าสู่ระบบแบบ **Auto-Provisioning ผ่าน Active Directory (AD)** โดยเมื่อพนักงานล็อกอินผ่าน AD ระบบจะสร้างบัญชีและกำหนดสิทธิ์ผู้ใช้งานทั่วไป (`User`) ให้อัตโนมัติทันที สามารถออกเลขได้เลยโดยไม่ต้องรอ Admin เซ็ตสิทธิ์ล่วงหน้า | High |
| **BR-EDR-004** | **EDR Memo** | ระบบรองรับการระบุและจัดการลิงก์เอกสารอ้างอิง (SharePoint URL / ลิงก์ภายนอก) สามารถเพิ่ม, แก้ไข และลบลิงก์ได้ตลอดเวลา พร้อมบันทึกประวัติลงใน Audit Log | Medium |
| **BR-EDR-005** | **EDR Memo** | ผู้ขอ หรือ Admin ฝ่าย สามารถกด "ปิดเลขเอกสาร (Close Document)" พร้อมระบุเหตุผลเพื่อเปลี่ยนสถานะเป็น `Closed` เมื่อใช้งานเสร็จสิ้น | High |
| **BR-EDR-006** | **EDR Memo** | ระบบแบ่งแยกระดับสิทธิ์ระหว่าง **SuperAdmin** (ดูแลระบบและ Master Data ทั้งหมด) และ **Admin ฝ่าย (Department Admin)** (เห็นและจัดการงานของทุกคนในฝ่ายตนเอง 100% แต่ **ไม่มีสิทธิ์เข้าถึงเมนูตั้งค่าระบบ Settings และ Master Data**) | High |
| **BR-EDR-007** | **EDR Memo** | ระบบมีฟังก์ชัน **ปุ่มคัดลอกเลขที่เอกสาร (One-Click Copy Document Number)** ลง System Clipboard เพียงคลิกเดียวในหน้าต่างสำเร็จ, หน้ารายละเอียดคำขอ และตารางรายการ | High |
| **BR-EDR-008** | **EDR Memo** | ระบบบันทึก **Audit Log Trail** ประวัติการกระทำสำคัญ: เมื่อออกเลขใหม่, เมื่อมีการยกเลิกเลขเอกสาร (Cancel Document) พร้อมบังคับระบุเหตุผล, และเมื่อมีการเพิ่ม/แก้ไข/ลบ ลิงก์เอกสารอ้างอิง | High |
...
| **การเลือกประเภทเอกสารบันทึกภายใน (Internal Doc Types)** | ระบบที่ 1 (EDR Memo) | ตัดเรื่องการเลือกประเภทเอกสาร, Quick Add, และ Master DocTypes ออกจากระบบบันทึกภายในทั้งหมด เพื่อความคล่องตัวสูงสุด |
...
ระบบขอเลขบันทึกภายในถูกออกแบบบนหลักการ **"ความคล่องตัวสูงสุดในการปฏิบัติงาน (Operational Agility)"** โดยมีหัวใจสำคัญดังนี้:
1. **การบริการตนเองแบบออกเลขทันที (Direct Self-Service Number Issuance):** พนักงานทุกคนสามารถขอเลขที่เอกสารบันทึกข้อความและได้รับเลขทันที **โดยไม่มีสายการอนุมัติ (No Approval Flow)**, **ไม่มีการมอบหมายงาน (No Task Assignment)**, และ **ไม่มีการเลือกประเภทเอกสาร (No Document Type Selection)** ให้เกิดความล่าช้า
2. **การเข้าใช้งานแบบ Auto-Provisioning ผ่าน Active Directory (AD):** พนักงานทั่วไป **ไม่ต้องรอให้ Admin เซ็ตสิทธิ์ล่วงหน้า** เมื่อล็อกอินผ่าน Windows Authentication / AD ระบบจะอ่าน Claims สังกัดและสร้างบัญชีผู้ใช้กำหนดสิทธิ์เป็น `User ทั่วไป` ให้อัตโนมัติทันทีตั้งแต่การเข้าใช้งานครั้งแรก
3. **การแบ่งแยกระดับการบริหารงาน (Administrative Tiering):** แยกหน้าที่ระหว่าง **`SuperAdmin`** (ผู้ดูแลระบบสูงสุด จัดการ Master Data โครงสร้างองค์กร, รูปแบบเลข, รหัสฝ่าย, จัดการสิทธิ์ Admin ฝ่าย และดู Audit Log ทั้งระบบ) ออกจาก **`Admin ฝ่าย (Department Admin)`** (ผู้ดูแลประจำฝ่ายที่มองเห็นและติดตามงานของทุกคนในฝ่ายตนเอง 100% แต่ **ถูกตัดสิทธิ์จากการเข้าถึงเมนูตั้งค่าระบบ Settings และ Master Data โดยเด็ดขาด**)

---

#### 3.1.2 คำอธิบายบทบาทผู้ใช้อย่างละเอียดของระบบ EDR Memo (EDR Role Catalog)

ระบบ EDR Memo มีบทบาทผู้ใช้งาน **3 บทบาทหลัก** ดังนี้:

##### 1. EDR-R01: ผู้ใช้งานทั่วไป (General User / Memo Requester)
- **กลุ่มผู้ใช้งาน:** พนักงานประจำและพนักงานสัญญาจ้างทุกคนใน บมจ. เทเวศประกันภัย โดยได้รับสิทธิ์อัตโนมัติเมื่อเข้าสู่ระบบผ่าน Windows Authentication / Active Directory (JIT Auto-Provisioning)
- **ขอบเขตข้อมูล (Data Scope):** **ระดับบุคคล (Personal Scope)** — เข้าถึงได้เฉพาะคำขอเลขที่ตนเองเป็นผู้สร้างเท่านั้น (`WHERE CreatedBy = CurrentUser.Username`)
- **หน้าที่และสิทธิ์ที่ทำได้ (Detailed Capabilities & Allowed Actions):**
  1. เข้าสู่ระบบผ่าน AD และระบบทำการ Auto-provision ข้อมูลสังกัดและสิทธิ์ให้ทันทีโดยไม่ต้องรอ Admin อนุมัติ
  2. เข้าสู่เมนู "ขอสร้างเลขเอกสารบันทึกภายใน" (Create Memo Request)
  3. เลือกโครงสร้างสังกัด 3 ระดับ (สายงาน Line $\rightarrow$ ฝ่าย Department $\rightarrow$ หน่วยงานย่อย Unit/Team) และดู Real-time Preview รูปแบบเลขที่จะได้รับตาม Master
  4. กรอกชื่อเรื่อง และระบุลิงก์เอกสารต้นฉบับ (SharePoint Online / Intranet URL) พร้อมกดปุ่มทดสอบเปิดลิงก์
  ...
  6. **กดปุ่มคัดลอกเลขที่เอกสาร (One-Click Copy Document Number)** จากหน้าต่างออกเลขสำเร็จ, หน้ารายละเอียดคำขอ หรือตารางรายการ เพื่อคัดลอกเลขลง System Clipboard ไปใช้งานได้ทันที
  7. ดูรายละเอียดคำขอเลขที่ได้รับ (Document Detail Screen) ของตนเอง
  8. **จัดการลิงก์เอกสารอ้างอิง (Document Links Management):** เพิ่มลิงก์ (Add Link), แก้ไขลิงก์ (Update Link), หรือลบลิงก์ (Remove Link) ของคำขอตนเองได้ตลอดเวลา โดยระบบจะบันทึก Audit Log ทุกครั้ง
  9. กด **"ปิดเลขเอกสาร (Close Document)"** พร้อมระบุเหตุผลบังคับ เมื่อพิมพ์หนังสือและนำเลขไปใช้งานเสร็จสิ้นแล้ว
  10. กด **"ยกเลิกเลขเอกสาร (Cancel Document)"** พร้อมระบุเหตุผลบังคับ $\ge 10$ ตัวอักษร กรณีขอเลขผิด พิมพ์ผิด หรือไม่ได้ใช้งาน โดยระบบจะเปลี่ยนสถานะเป็น `Cancelled` ล็อกข้อมูลเป็น Read-only และบันทึก Audit Log
  11. ค้นหาประวัติคำขอเลขบันทึกภายในของตนเองในหน้าค้นหา (Search Screen)
- **สิ่งที่ระบบ EDR Memo ไม่มี และบทบาทนี้ทำไม่ได้ (Explicit System Boundaries & Limitations):**
  - ❌ **ไม่มีการเลือกประเภทเอกสาร (No DocType Selection):** ระบบยกเลิกประเภทเอกสารและ Quick Add ทั้งหมดแล้ว
  ...
  - ❌ **ไม่สามารถเข้าเมนูตั้งค่าระบบ (Settings / Master Data):** ไม่มีสิทธิ์เข้าถึงเมนูตั้งค่าใดๆ

##### 2. EDR-R02: Admin ฝ่าย (Department Admin)
- **กลุ่มผู้ใช้งาน:** บุคคลที่ได้รับมอบหมายประจำฝ่าย (เช่น หัวหน้าฝ่าย, เลขานุการฝ่าย หรือธุรการฝ่าย) โดยได้รับการแต่งตั้งสิทธิ์จาก SuperAdmin
- **ขอบเขตข้อมูล (Data Scope):** **ระดับฝ่าย (Department Scope)** — เข้าถึงคำขอเลขบันทึกภายในของพนักงานทุกคนในฝ่ายที่ตนสังกัด (`WHERE DeptCode = CurrentUser.DepartmentCode`)
- **หน้าที่และสิทธิ์ที่ทำได้ (Detailed Capabilities & Allowed Actions):**
  1. เข้าดู Dashboard และรายการคำขอเลขบันทึกภายในของ **ทุกคนในฝ่ายตนเอง 100%**
  2. ค้นหาคำขอเลขบันทึกภายในขั้นสูงของทุกคนในฝ่าย
  3. **กดปุ่มคัดลอกเลขเอกสาร (One-Click Copy)** ของคำขอในฝ่ายตนเองเพื่อนำไปใช้งานได้อย่างรวดเร็ว
  4. **จัดการลิงก์เอกสารอ้างอิงแทนพนักงานในฝ่าย:** เพิ่มลิงก์, แก้ไขลิงก์, หรือลบลิงก์ (ระบบบันทึก Audit Log ระบุชื่อ Admin ฝ่ายเป็นผู้ดำเนินการ)
  5. กด **"ปิดเลขเอกสาร (Close Document)"** แทนพนักงานในฝ่าย พร้อมระบุเหตุผลบังคับ (เช่น กรณีพนักงานลาออก หรือลืมปิดเลข)
  6. กด **"ยกเลิกเลขเอกสาร (Cancel Document)"** แทนพนักงานในฝ่าย พร้อมระบุเหตุผลบังคับ $\ge 10$ ตัวอักษร (เช่น กรณีขอเลขซ้ำซ้อน) พร้อมบันทึก Audit Log
  7. เข้าดูและพิมพ์รายงานสถิติการออกเลขประจำเดือนและประจำปีของฝ่ายตนเอง (Report Screen)
  8. ขอสร้างเลขบันทึกภายในสำหรับงานของตนเองได้เช่นเดียวกับ EDR-R01
- **สิ่งที่ระบบ EDR Memo ไม่มี และบทบาทนี้ทำไม่ได้ (Explicit System Boundaries & Limitations):**
  - ❌ **ไม่มีสิทธิ์เข้าถึงเมนูตั้งค่าระบบและการจัดการ Master Data โดยเด็ดขาด (NO SYSTEM SETTINGS ACCESS):**  
    - ไม่สามารถเข้าเมนู Settings ใดๆ ในระบบ
    - ไม่สามารถดูหรือแก้ไข Master สายงาน (Lines), ฝ่าย (Departments), หน่วยงานย่อย (Units), รูปแบบเลข (Number Formats), หรือตัวย่อฝ่าย (Department Codes)
    - ระบบจะซ่อนเมนู Settings ออกจาก Navigation Bar สำหรับบทบาทนี้ และหากพยายามเข้าถึง URL ตั้งค่าโดยตรง ระบบจะส่งกลับ **HTTP 403 Forbidden** ทันที
  - ❌ **ไม่มีหน้าที่กดอนุมัติเลข (No Approval Role):** Admin ฝ่ายไม่มีสิทธิ์กักหรือ Reject เลขบันทึกภายใน
  - ❌ **ไม่สามารถเข้าถึงคำขอของฝ่ายอื่นได้:** ถูกจำกัดสิทธิ์เฉพาะฝ่ายที่ตนสังกัดเท่านั้น

##### 3. EDR-R03: SuperAdmin (EDR SuperAdmin)
- **กลุ่มผู้ใช้งาน:** เจ้าหน้าที่ธุรการส่วนกลาง / ผู้ดูแลระบบสารสนเทศระดับสูงส่วนงาน EDR
...
- **หน้าที่และสิทธิ์ที่ทำได้ (Detailed Capabilities & Allowed Actions):**
  ...
  4. **กำหนดและจัดการสิทธิ์ Admin ฝ่าย (Assign / Revoke Department Admin Role):** มอบหมายหรือเพิกถอนสิทธิ์ Admin ฝ่ายให้แก่พนักงานในแต่ละฝ่าย
  5. ตรวจสอบ **Audit Log Trail ทั้งระบบ:** เรียกดูประวัติการออกเลข, การยกเลิกเลขเอกสาร, การเพิ่ม/แก้ไข/ลบ ลิงก์เอกสาร และการปิดเลขทั้งหมดของทุกฝ่ายในระบบ
  6. ดูรายงานสถิติภาพรวมการออกเลขบันทึกภายในของทุกฝ่ายทั่วทั้งบริษัท
  7. ดำเนินการแก้ไขลิงก์, ปิดเลข หรือยกเลิกเลขเอกสารได้ทุกคำขอในระบบ (กรณีเกิดข้อผิดพลาดฉุกเฉิน)
- **สิ่งที่ระบบ EDR Memo ไม่มี และบทบาทนี้ทำไม่ได้ (Explicit System Boundaries & Limitations):**
  - ❌ **ดูแลเฉพาะระบบ EDR Memo:** ไม่ได้ทำหน้าที่บริหารจัดการระบบงานสารบรรณรับเข้า-ส่งออก (Correspondence System)

---

#### 3.1.3 ตารางสรุปสิทธิ์การใช้งานเฉพาะระบบ EDR Memo (EDR Memo Permission Matrix)

| ฟังก์ชันงานของระบบ EDR Memo | ผู้ใช้งานทั่วไป (EDR-R01) | Admin ฝ่าย (EDR-R02) | SuperAdmin (EDR-R03) |
|---|:---:|:---:|:---:|
| **เข้าสู่ระบบผ่าน AD อัตโนมัติ (Auto-Provisioning)** | ✅ (ไม่ต้องเซ็ตสิทธิ์) | ✅ (SuperAdmin กำหนด Role) | ✅ (SuperAdmin Role) |
...
| **ออกเลขทันทีแบบ Atomic Lock (No Approval Flow)** | ✅ | ✅ | ✅ |
| **ปุ่มคัดลอกเลขเอกสาร (One-Click Copy Clipboard)** | ✅ | ✅ | ✅ |
| **ดูรายละเอียดคำขอเลขที่ได้รับ (Detail Screen)** | ✅ (เฉพาะคำขอตนเอง) | ✅ (ทุกคนในฝ่าย) | ✅ (ทุกฝ่ายทั้งบริษัท) |
| **จัดการลิงก์เอกสาร (เพิ่ม / แก้ไข / ลบ Link)** | ✅ (เฉพาะคำขอตนเอง) | ✅ (คำขอในฝ่าย) | ✅ (ทุกคำขอ) |
| **กดยกเลิกเลขเอกสารพร้อมระบุเหตุผล (Cancel Document)** | ✅ (เฉพาะคำขอตนเอง) | ✅ (คำขอในฝ่าย) | ✅ (ทุกคำขอ) |
| **กดปิดเลขเอกสารพร้อมระบุเหตุผล (Close Document)** | ✅ (เฉพาะคำขอตนเอง) | ✅ (คำขอในฝ่าย) | ✅ (ทุกคำขอ) |
...
| **เข้าถึงเมนูตั้งค่าระบบ (System Settings Menu)** | ❌ (ซ่อนเมนู / 403) | ❌ (ซ่อนเมนู / 403) | ✅ |
| **จัดการ Master สายงาน, ฝ่าย, หน่วยงานย่อย** | ❌ | ❌ | ✅ |
| **จัดการ Master รหัสย่อฝ่าย และ Running Format** | ❌ | ❌ | ✅ |
| **แต่งตั้งและเพิกถอนสิทธิ์ Admin ฝ่าย** | ❌ | ❌ | ✅ |
| **ตรวจสอบ Audit Log ประวัติยกเลิก/ลิงก์/ออกเลข** | ❌ | ❌ | ✅ (ทั้งระบบ 10 ปี) |

---

#### 3.1.4 กฎการควบคุมสิทธิ์และกลไก Active Directory Auto-Provisioning
1. **กลไกการ Auto-Provisioning สำหรับผู้ใช้งานทั่วไป (JIT Provisioning Flow):**  
   - เมื่อพนักงานล็อกอินเข้าสู่ระบบ EDR ผ่าน Windows Authentication / Active Directory:
     - ระบบตรวจสอบ Claims: `Username`, `FullName`, `Email`, `DepartmentCode`, `Title`
     - ระบบตรวจสอบในตาราง `EDR_USER`: หากยังไม่พบ Username นี้ ระบบจะทำ **Auto-Provisioning** ทันที:
       `INSERT INTO EDR_USER (Username, FullName, DeptCode, RoleId, IsAutoProvisioned, CreatedDate) VALUES (@Username, @FullName, @DeptCode, 'USER', 1, GETDATE())`
     - ผู้ใช้เข้าสู่ระบบได้ทันทีในฐานะ `EDR-R01 (General User)` โดยไม่ต้องรอการอนุมัติหรือเซ็ตสิทธิ์จาก Admin ใดๆ ทั้งสิ้น
2. **ระดับบุคคล (Personal Scope - EDR-R01):**  
   ระบบกรองข้อมูลคำขอด้วยเงื่อนไข SQL: `WHERE CreatedBy = CurrentUser.Username`
3. **ระดับฝ่าย (Department Scope - EDR-R02):**  
   ระบบกรองข้อมูลคำขอด้วยเงื่อนไข SQL: `WHERE DeptCode = CurrentUser.DepartmentCode`
4. **ระดับองค์กร (System-wide Scope - EDR-R03):**  
...
- **สถานะ `Draft` (ก่อนกด Confirm):** ผู้ขอสามารถแก้ไขข้อมูล สังกัด ชื่อเรื่อง และลิงก์เอกสารได้อิสระ
- **สถานะ `Created` (ออกเลขสำเร็จแล้ว):**  
  - ล็อกข้อมูลสังกัด, วันที่ออกเลข และเลขที่เอกสารเป็น Read-only ห้ามแก้ไขเด็ดขาด
  - **อนุญาตให้กดปุ่ม Copy เลขเอกสาร** เพื่อนำเลขไปพิมพ์บันทึกข้อความได้ตลอดเวลา
  - **อนุญาตให้จัดการลิงก์เอกสารอ้างอิง:** เพิ่มลิงก์ (Add), แก้ไขลิงก์ (Edit), ลบลิงก์ (Delete) โดยระบบบันทึก Audit Log ทุกครั้ง
  - **อนุญาตให้กด `Close Document`:** เพื่อปิดเลขเมื่อใช้งานเสร็จสิ้น (ต้องระบุเหตุผล)
  - **อนุญาตให้กด `Cancel Document`:** เพื่อยกเลิกเลขกรณีขอผิดหรือไม่ได้ใช้งาน (ต้องระบุเหตุผล $\ge 10$ ตัวอักษร) พร้อมบันทึก Audit Log
- **สถานะ `Cancelled` (ยกเลิกเลขแล้ว):**  
  - ล็อกข้อมูลทุกฟิลด์เป็น Read-only 100% แสดงแถบสถานะสีแดง `"ยกเลิกแล้ว (Cancelled)"`
  - แสดงเหตุผลการยกเลิก, ผู้กดยกเลิก, และวันเวลายกเลิก
  - บันทึกลงใน Audit Log ถาวร ไม่สามารถนำเลขกลับมาใช้งานใหม่ได้
- **สถานะ `Closed` (ปิดเลขแล้ว):**  
  - ล็อกข้อมูลทุกฟิลด์เป็น Read-only 100% ไม่สามารถแก้ไขลิงก์หรือเปิดกลับมาใช้งานใหม่ได้ แสดงเหตุผลและวันเวลาปิดเลขสมบูรณ์
...
|---|---|---|
| **1. รูปแบบสิทธิ์หลัก (Core Model)** | **Direct Self-Service Number Issuing** (ออกเลขเร็ว ไม่ต้องเลือกประเภทเอกสาร) | **Multi-Party Operational Workflow & Custody** (ควบคุมวงจรชีวิต) |
| **2. จำนวนบทบาทหลัก** | **3 บทบาท** (EDR-R01 ผู้ใช้ทั่วไป Auto-Provisioned, EDR-R02 Admin ฝ่าย ดูแลในฝ่ายแต่ห้ามเข้า Settings, EDR-R03 SuperAdmin สิทธิ์สูงสุด) | **7 บทบาท** (CORR-R01 ถึง CORR-R07) |
...
| **6. การติดตามเอกสารตัวจริง** | **ไม่มี** (ระบบเก็บเฉพาะ SharePoint URL / ลิงก์เอกสาร) | **มี Stateful Chain of Custody** บันทึกผู้ถือครองกระดาษตัวจริงทุกขณะ |
| **7. การคำนวณ Progress %** | **ไม่มี** (สถานะแบบ Flat: Draft $\rightarrow$ Created $\rightarrow$ Cancelled / Closed) | **มี Progress Engine** คำนวณความคืบหน้ารวมเฉลี่ยถ่วงน้ำหนักทุกฝ่าย |
| **8. การรักษาความปลอดภัยไฟล์** | ควบคุมสิทธิ์ตามโฟลเดอร์ของ SharePoint Online | **มี Top Secret OTP Gate** ล็อกไฟล์แนบ ขอ OTP 6 หลักทางอีเมล |
| **9. บทบาทเฝ้าติดตาม (Watcher)** | ไม่มี (ดูได้เฉพาะ Admin ฝ่าย และ SuperAdmin) | **มี Monitor Watcher (CORR-R05)** ติดตามและกด Follow up ได้โดยไม่แก้ข้อมูล |
| **10. การปิดงาน / ยกเลิกงาน** | ผู้ขอ หรือ Admin ฝ่าย กด **"Close Document"** หรือ **"Cancel Document"** พร้อมเหตุผลบังคับและ Audit Log | ทุกฝ่ายกด **"Complete"** จนครบ 100% หรือผู้ส่งออกกด **"Delivered"** |
...
- **ความแตกต่างสำคัญด้านการ Provisioning ผู้ใช้งาน:**
  - **ระบบที่ 1 (EDR Memo):** รองรับ **JIT Auto-Provisioning** เต็มรูปแบบ พนักงานทุกคนในองค์กรเมื่อล็อกอินด้วย AD ครั้งแรก ระบบจะสร้างบัญชีและให้สิทธิ์ `EDR-R01 (General User)` ทันทีโดยไม่ต้องรอ Admin อนุมัติสิทธิ์ล่วงหน้า (ส่วน `EDR-R02` และ `EDR-R03` SuperAdmin เป็นผู้กำหนดสิทธิ์)
  - **ระบบที่ 2 (Correspondence System):** ผู้ใช้ต้องผ่านการตั้งค่าและเปิดสิทธิ์การใช้งาน (User Provisioning) จากเจ้าหน้าที่ Admin ในหน้าจอตั้งค่าก่อน จึงจะเข้าใช้งานได้
...
```mermaid
flowchart TB
    U_REQ([ผู้ใช้งานทั่วไป<br/>EDR-R01 - Auto-Provisioned])
    U_HEAD([Admin ฝ่าย<br/>EDR-R02 - Dept Admin])
    U_ADM([SuperAdmin<br/>EDR-R03 - SuperAdmin])
    ...
    subgraph EDR_MEMO_SYSTEM["[ระบบที่ 1] ระบบขอเลขบันทึกภายใน (EDR Internal Memo)"]
        ...
        UC1_3[คัดลอกเลขที่เอกสาร One-Click Copy Clipboard]
        UC1_4[จัดการลิงก์เอกสาร Add/Edit/Delete Link พร้อม Audit Log]
        UC1_5[ยกเลิกเลขเอกสารพร้อมระบุเหตุผล Cancel Document พร้อม Audit Log]
        UC1_6[ปิดเลขเอกสารพร้อมระบุเหตุผล Close Document]
        UC1_7[ค้นหาเลขบันทึกภายใน Search]
        UC1_8[ดูรายงานสถิติการขอเลข Report]
        UC1_9[จัดการ Master Hierarchy, Formats & Codes SuperAdmin Only]
        UC1_10[แต่งตั้ง/เพิกถอนสิทธิ์ Admin ฝ่าย SuperAdmin Only]
        UC1_11[ตรวจสอบ Audit Log ทั่วทั้งระบบ SuperAdmin Only]
    end

    U_REQ --> UC1_1 & UC1_2 & UC1_3 & UC1_4 & UC1_5 & UC1_6 & UC1_7
    U_HEAD --> UC1_1 & UC1_2 & UC1_3 & UC1_4 & UC1_5 & UC1_6 & UC1_7 & UC1_8
    U_ADM --> UC1_1 & UC1_2 & UC1_3 & UC1_4 & UC1_5 & UC1_6 & UC1_7 & UC1_8 & UC1_9 & UC1_10 & UC1_11
```
...
| **ลำดับขั้นตอนการดำเนินงาน (Order of Operations)** | **1.** ล็อกอินผ่าน AD $\rightarrow$ ระบบ Auto-Provisioning สิทธิ์ User ทั่วไปให้อัตโนมัติทันที<br/>**2.** เลือกสังกัด (สายงาน $\rightarrow$ ฝ่าย $\rightarrow$ ทีม)<br/>**3.** ระบบ Preview เลขที่จะได้รับตาม Format Real-time<br/>**4.** กรอกชื่อเรื่อง และระบุ SharePoint URL (หรือลิงก์เอกสารภายนอก)<br/>**5.** กดยืนยันข้อมูลในหน้าต่าง Confirmation<br/>**6.** **ระบบออกเลขทันที (Instant Atomic Generation)** สถานะเป็น `Created` (บันทึก Audit Log)<br/>**7.** **กดปุ่มคัดลอกเลขที่เอกสาร (One-Click Copy)** เพื่อนำเลขไปใส่ในบันทึกข้อความได้ทันที<br/>**8.** ผู้ขอ/Admin ฝ่าย สามารถเข้ามา **เพิ่ม/แก้ไข/ลบ ลิงก์เอกสาร** ได้ตลอดเวลา (บันทึก Audit Log)<br/>**9.** หากพิมพ์ผิดหรือไม่ได้ใช้งาน สามารถกด **"ยกเลิกเลขเอกสาร (Cancel Document)"** พร้อมระบุเหตุผล (สถานะเป็น `Cancelled` บันทึก Audit Log)<br/>**10.** เมื่อใช้เสร็จ กด "ปิดเลขเอกสาร (Close Document)" พร้อมระบุเหตุผล สถานะเป็น `Closed` | **1.** Register รับเข้า + แนบไฟล์/ถ่ายภาพ WebRTC Camera + กำหนด Deadline<br/>**2.** Assign งานไปยังฝ่าย/บุคคล (Multiple Select ได้หลายฝ่าย)<br/>**3.** ระบบส่ง Notification 3 ช่องทาง<br/>**4.** ผู้รับงานปลายทางกด **"Accept (รับงาน)"** เพื่อยืนยันการรับงานจริง และยืนยันการถือครองเอกสารตัวจริง (Chain of Custody)<br/>**5.** ผู้รับงานปฏิบัติงาน หรือ **"Onward Delegate"** มอบหมายต่อให้ลูกทีมในฝ่าย<br/>**6.** เมื่อปฏิบัติงานเสร็จ ผู้รับงานกด **"ปิดงาน (Complete)"**<br/>**7.** Progress Engine คำนวณความคืบหน้ารวมจนครบ 100% เอกสารเปลี่ยนสถานะเป็น `Completed` | **1.** สร้างคำขอออกเลขส่งออก (Register Outgoing)<br/>**2.** เลือกลักษณะการออกเลขผ่าน EDR API (Flow A ทันที หรือ Flow B รออนุมัติผู้บริหาร)<br/>**3.** ระบบสร้างเลขคู่ขนาน Dual Running Numbers (เลขไทย เช่น `ทว.xxx/2569` และเลขอังกฤษ เช่น `DVS.xxx/2026`)<br/>**4.** บังคับแนบไฟล์เอกสารที่ลงนามแล้ว<br/>**5.** บันทึกการส่งหนังสือ (Sent) + เลขพัสดุ ปณ.<br/>**6.** บันทึกหลักฐานใบตอบรับปลายทาง (Delivered)<br/>**7.** ปิดงานสมบูรณ์ (`Completed`) |
...
sequenceDiagram
    autonumber
    actor User as ผู้ใช้งานทั่วไป / Admin ฝ่าย
    participant UI as Web Application (UI)
    participant API as EDR Internal API
    participant AD as Active Directory
    participant DB as SQL Server Database

    Note over User,DB: ขั้นตอนที่ 1: เข้าสู่ระบบแบบ AD Auto-Provisioning (ไม่ต้องเซ็ตสิทธิ์ล่วงหน้า)
    User->>UI: ล็อกอินผ่าน Windows AD Authentication
    UI->>API: POST /api/auth/ad-login
    API->>AD: ยืนยันตัวตน และดึง Claims (Username, FullName, DeptCode)
    AD-->>API: AD Claims Payload
    rect rgb(245, 255, 250)
        Note over API,DB: JIT Auto-Provisioning สำหรับผู้ใช้ใหม่
        API->>DB: ตรวจสอบ Username ใน EDR_USER
        alt ผู้ใช้ใหม่ (First Login)
            API->>DB: INSERT INTO EDR_USER (Role='USER', IsAutoProvisioned=1)
        end
    end
    API-->>UI: JWT Session Token (Role: EDR-R01/R02/R03)

    Note over User,DB: ขั้นตอนที่ 2: เปิดฟอร์มขอเลขและแสดง Real-time Preview (ไม่มีประเภทเอกสาร)
    User->>UI: เข้าเมนู "ขอสร้างเลขเอกสารบันทึกภายใน"
    UI->>API: GET /EDR/InternalRequest/Create
    API->>DB: โหลด Master Units ในฝ่าย + Running Pattern ตาม Scope
    DB-->>API: Master Hierarchy Payload
    API-->>UI: แสดงฟอร์มขอเลข พร้อม Preview เลข Real-time ตามสังกัด

    Note over User,DB: ขั้นตอนที่ 3: กรอกข้อมูลและยืนยันคำขอ
    User->>UI: กรอกชื่อเรื่อง (Subject), ระบุ SharePoint URL หรือลิงก์เอกสาร
    User->>UI: กดปุ่ม "ดำเนินการต่อ" -> แสดงหน้าต่างยืนยันข้อมูล (Confirmation Modal)
    User->>UI: กดยืนยันการออกเลข (Confirm)

    Note over User,DB: ขั้นตอนที่ 4: ระบบออกเลขทันทีแบบ Atomic Lock (No Approval) & One-Click Copy
    UI->>API: POST /EDR/InternalRequest/Create (Payload)
    rect rgb(240, 248, 255)
        Note over API,DB: Atomic Counter Increment & Audit Log
        API->>DB: Lock & Increment Counter (+1) ตาม Scope (Line/Dept/Unit)
        API->>DB: INSERT INTO INTERNAL_DOC_REQUEST (Status='Created')
        API->>DB: INSERT INTO AUDIT_LOG (Action='CREATE_MEMO', DocumentNo)
    end
    DB-->>API: Transaction Committed
    API-->>UI: 201 Created + เลขที่เอกสารที่ได้รับ
    UI-->>User: แสดง Success Dialog พร้อมปุ่ม "คัดลอกเลขที่เอกสาร (Copy Number)"
    User->>UI: คลิกปุ่ม "คัดลอกเลขที่เอกสาร" -> บันทึกลง System Clipboard + แสดง Toast สำเร็จ

    Note over User,DB: ขั้นตอนที่ 5: การจัดการลิงก์เอกสาร (Add/Edit/Delete Link) พร้อม Audit Log
    User->>UI: เพิ่ม/แก้ไข/ลบ SharePoint URL ในหน้ารายละเอียด
    UI->>API: POST /EDR/InternalRequest/ManageLink (Id, ActionType, NewUrl)
    API->>DB: UPDATE INTERNAL_DOC_REQUEST (DocumentUrl) + INSERT AUDIT_LOG (Action='UPDATE_DOCUMENT_LINK')
    DB-->>API: Success
    API-->>UI: อัปเดตลิงก์สำเร็จ

    Note over User,DB: ขั้นตอนที่ 6: การยกเลิกเลขเอกสาร (Cancellation) หรือปิดเลข (Closure)
    alt กรณีขอยกเลิกเลข (พิมพ์ผิด / ไม่ได้ใช้งาน)
        User->>UI: กดปุ่ม "ยกเลิกเลขเอกสาร" -> กรอกเหตุผลบังคับ (>= 10 ตัวอักษร)
        UI->>API: POST /EDR/InternalRequest/Cancel (Id, CancelReason)
        API->>DB: UPDATE Status='Cancelled' + INSERT AUDIT_LOG (Action='CANCEL_DOCUMENT')
        DB-->>API: Cancelled Success
        API-->>UI: แสดงสถานะเอกสารเป็น Cancelled (Read-only ถาวร)
    else กรณีใช้งานเสร็จสิ้น (Closure)
        User->>UI: กดปุ่ม "ปิดเลขเอกสาร" -> กรอกเหตุผลการปิดเลข
        UI->>API: POST /EDR/InternalRequest/Close (Id, CloseReason)
        API->>DB: UPDATE Status='Closed' + INSERT AUDIT_LOG (Action='CLOSE_DOCUMENT')
        DB-->>API: Closed Success
        API-->>UI: แสดงสถานะเอกสารเป็น Closed สมบูรณ์
    end
```
...
- **ฟังก์ชันการทำงาน:** ฟอร์มเข้าสู่ระบบด้วยบัญชี Windows Active Directory / LDAP โดยมีระบบ **Active Directory Auto-Provisioning** รองรับผู้ใช้งานทั่วไปเข้าสู่ระบบได้ทันที ระบบจะสร้าง User Profile และผูกสังกัดให้อัตโนมัติโดยไม่ต้องรอ Admin ตั้งค่าสิทธิ์ล่วงหน้า
...
- **ฟังก์ชันการทำงาน:** สรุปสถิติคำขอเลขบันทึกภายใน, รายการคำขอล่าสุด, สถานะคำขอ (Created / Cancelled / Closed) พร้อมทางลัดเข้าสู่หน้าสร้างคำขอ ในตารางรายการมี **ปุ่มคัดลอกเลขที่เอกสาร (One-Click Copy)** ประจำแต่ละแถว เพื่อให้คัดลอกเลขไปใช้งานได้ทันที (สิทธิ์การมองเห็น: ผู้ใช้ทั่วไปเห็นเฉพาะงานตนเอง, Admin ฝ่ายเห็นทุกคนในฝ่าย 100%, SuperAdmin เห็นทั้งองค์กร)
...
- **ฟังก์ชันการทำงาน:** ตารางแสดงรายการเลขบันทึกภายในที่ออกแล้ว กรองตามช่วงวันที่ สถานะ (`Created`, `Cancelled`, `Closed`) และค้นหาตามชื่อเรื่องหรือเลขที่เอกสาร พร้อมปุ่มคัดลอกเลขเอกสาร (One-Click Copy) ในทุกแถว
...
- **ฟังก์ชันการทำงาน:** ฟอร์มบันทึกข้อมูลขอเลข แสดง Real-time Running Number Preview ตามสายงาน/ฝ่าย/ทีมที่เลือก **โดยไม่มีช่องเลือกประเภทเอกสาร** กรอกเฉพาะชื่อเรื่อง และระบุลิงก์เอกสารอ้างอิง (Optional)

#### 5. [ยกเลิกการใช้งาน - Deprecated / Removed] Searchable Dropdown เลือกประเภทเอกสาร (DocType Dropdown)
- **ภาพหน้าจอเดิม:**  
  ![05a_doctype_dropdown.png](./docs/images/edr_memo/05a_doctype_dropdown.png)  
  *(ขณะเปิด Dropdown เดิม: [04a_doctype_dropdown_opened.png](./docs/images/edr_memo/04a_doctype_dropdown_opened.png))*
- **สถานะเชิงข้อกำหนด (V3.3.0):** **ยกเลิกการใช้งาน (Deprecated / Removed)** — ตัดฟังก์ชันการเลือกประเภทเอกสารออกจากระบบขอเลขบันทึกภายในทั้งหมด เพื่อความสะดวกรวดเร็วในการทำงาน
...
- **ฟังก์ชันการทำงาน:** แสดงข้อมูลชื่อเรื่อง, ลิงก์ SharePoint URL ที่ผ่านการตรวจสอบ HTTPS และปุ่มดำเนินการต่อ (ไม่ปรากฏฟิลด์ประเภทเอกสาร)

#### 7. [ยกเลิกการใช้งาน - Deprecated / Removed] Quick Add Modal เพิ่มประเภทเอกสารด่วน (Quick Add Modal)
- **ภาพหน้าจอเดิม:**  
  ![05c_quick_add_modal.png](./docs/images/edr_memo/05c_quick_add_modal.png)
- **สถานะเชิงข้อกำหนด (V3.3.0):** **ยกเลิกการใช้งาน (Deprecated / Removed)** — ตัดฟังก์ชัน Quick Add ประเภทเอกสารออกจากระบบบันทึกภายในทั้งหมด

#### 8. หน้ายืนยันข้อมูลก่อนออกเลข และแจ้งเตือนสำเร็จ (Confirmation Step & Success Dialog)
- **ภาพหน้าจอ:**  
  ![19_create_confirmation.png](./docs/images/edr_memo/19_create_confirmation.png)
- **ฟังก์ชันการทำงาน:** หน้าต่างทบทวนข้อมูลก่อนยืนยัน เมื่อกดยืนยันระบบจะออกเลขทันที (Instant Atomic Issuance) และแสดง Success Dialog พร้อม **ปุ่มคัดลอกเลขที่เอกสาร (One-Click Copy)** แสดงเด่นชัดคู่กับเลขที่เอกสาร เมื่อคลิกจะคัดลอกเลขลง System Clipboard ทันทีพร้อมแสดง Toast Notification `"คัดลอกเลขที่เอกสาร [xxxx] สำเร็จ"`
...
- **ฟังก์ชันการทำงาน:**  
  1. แสดงรายละเอียดเลขที่เอกสารที่ได้รับ พร้อม **ปุ่มคัดลอกเลขที่เอกสาร (One-Click Copy)** ข้าง Header เลขที่เอกสาร
  2. **ส่วนจัดการลิงก์เอกสารอ้างอิง (Document Links):** รองรับการเพิ่มลิงก์ (Add Link), แก้ไขลิงก์ (Update Link), หรือลบลิงก์ (Remove Link) พร้อมบันทึกประวัติลง Audit Log
  3. **ปุ่มปิดเลขเอกสาร (Close Document):** สำหรับปิดเลขเมื่อใช้งานเสร็จสิ้น
  4. **ปุ่มยกเลิกเลขเอกสาร (Cancel Document):** สำหรับขอยกเลิกเลขกรณีขอผิดหรือไม่ได้ใช้งาน (ต้องระบุเหตุผลบังคับ $\ge 10$ ตัวอักษร) พร้อมบันทึก Audit Log

#### 10. Modal ปิดเลขเอกสารและยกเลิกเลขเอกสาร (Close & Cancel Document Modals)
- **ภาพหน้าจอ:**  
  ![18_modal_close_document.png](./docs/images/edr_memo/18_modal_close_document.png)
- **ฟังก์ชันการทำงาน:**  
  - ป๊อปอัปยืนยันการปิดเลขเอกสาร บังคับกรอกเหตุผลการปิดเลข เพื่อเปลี่ยนสถานะเป็น `Closed`
  - ป๊อปอัปยืนยันการยกเลิกเลขเอกสาร (Cancel Document Modal) บังคับกรอกเหตุผลการยกเลิก $\ge 10$ ตัวอักษร เพื่อเปลี่ยนสถานะเป็น `Cancelled` และบันทึก Audit Log ถาวร
...
- **ฟังก์ชันการทำงาน:** ค้นหาเอกสารขั้นสูง กรองตามสายงาน ฝ่าย หน่วยงานย่อย ช่วงวันที่ และคำสำคัญ (Admin ฝ่ายค้นหาได้ทุกคนในฝ่าย, SuperAdmin ค้นหาได้ทั้งระบบ)
...
- **ฟังก์ชันการทำงาน:** สรุปรายงานการออกเลขบันทึกภายในประจำเดือน/ปี และส่งออกไฟล์ Excel (Admin ฝ่ายดูเฉพาะฝ่ายตนเอง, SuperAdmin ดูภาพรวมทุกฝ่ายทั่วทั้งบริษัท)

#### 13. หน้าจอจัดการ Master สายงาน [เฉพาะ SuperAdmin เท่านั้น] (Settings Lines)
- **ภาพหน้าจอ:**  
...
- **ฟังก์ชันการทำงาน:** จัดการข้อมูลสายงาน (Line) เพิ่ม แก้ไข ระงับการใช้งานสายงาน *(สิทธิ์เฉพาะ SuperAdmin — Admin ฝ่ายไม่สามารถเข้าถึงได้)*

#### 14. หน้าจอจัดการ Master หน่วยงานภายใน [เฉพาะ SuperAdmin เท่านั้น] (Settings Units)
- **ภาพหน้าจอ:**  
...
- **ฟังก์ชันการทำงาน:** จัดการหน่วยงานย่อย/ทีมภายในฝ่าย กำหนดตัวย่อทีมสำหรับใช้ในการ Running เลข *(สิทธิ์เฉพาะ SuperAdmin — Admin ฝ่ายไม่สามารถเข้าถึงได้)*

#### 15. หน้าจอผูกฝ่ายกับสายงาน [เฉพาะ SuperAdmin เท่านั้น] (Settings Departments)
- **ภาพหน้าจอ:**  
  ![11_settings_departments.png](./docs/images/edr_memo/11_settings_departments.png)
- **ฟังก์ชันการทำงาน:** จับคู่ฝ่าย (Department) ให้อยู่ภายใต้สายงาน (Line) เพื่อรองรับ Hierarchy 3 ระดับ *(สิทธิ์เฉพาะ SuperAdmin — Admin ฝ่ายไม่สามารถเข้าถึงได้)*

#### 16. หน้าจอจัดการตัวย่อฝ่ายและ Running Config [เฉพาะ SuperAdmin เท่านั้น] (Settings Department Codes)
- **ภาพหน้าจอ:**  
  ![12_settings_department_codes.png](./docs/images/edr_memo/12_settings_department_codes.png)
- **ฟังก์ชันการทำงาน:** กำหนดรหัสตัวย่อฝ่ายภาษาไทยและภาษาอังกฤษ เช่น `ฝสบ.` / `CAD` สำหรับขึ้นต้นเลขที่เอกสาร *(สิทธิ์เฉพาะ SuperAdmin — Admin ฝ่ายไม่สามารถเข้าถึงได้)*

#### 17. หน้าจอรูปแบบเลขเอกสารภายใน [เฉพาะ SuperAdmin เท่านั้น] (Settings Internal Number Formats)
- **ภาพหน้าจอ:**  
...
- **ฟังก์ชันการทำงาน:** กำหนด Scope การ Running เลข (ระดับ LINE / DEPT / UNIT) และรูปแบบ Pattern *(สิทธิ์เฉพาะ SuperAdmin — Admin ฝ่ายไม่สามารถเข้าถึงได้)*

#### 18. Modal แก้ไขรูปแบบเลขเอกสารภายใน [เฉพาะ SuperAdmin เท่านั้น] (Edit Internal Number Format Modal)
- **ภาพหน้าจอ:**  
  ![17_modal_edit_internal_number_format.png](./docs/images/edr_memo/17_modal_edit_internal_number_format.png)
- **ฟังก์ชันการทำงาน:** ป๊อปอัปปรับแต่งตัวแปร Pattern (เช่น `{DEPT_CODE}.{YEAR_TH}-{SEQ:6}`) *(สิทธิ์เฉพาะ SuperAdmin — Admin ฝ่ายไม่สามารถเข้าถึงได้)*

#### 19. [ยกเลิกการใช้งาน - Deprecated / Removed] หน้าจอจัดการ Master ประเภทเอกสาร (Settings Doc Types)
- **ภาพหน้าจอเดิม:**  
...
- **สถานะเชิงข้อกำหนด (V3.3.0):** **ยกเลิกการใช้งาน (Deprecated / Removed)** — ตัด Master ประเภทเอกสารและการอนุมัติ Quick Add ออกจากระบบ EDR Memo ทั้งหมด
...
erDiagram
    ...
    INTERNAL_DOC_REQUEST ||--o{ INTERNAL_DOC_LINK : "contains_links"
    ...
    INTERNAL_DOC_REQUEST ||--o{ AUDIT_LOG : "audits (cancel/link/close)"
```
...
1. **`INTERNAL_DOC_REQUEST`:** จัดเก็บคำขอเลขบันทึกภายใน (`Id`, `DocumentNo`, `Subject`, `LineId`, `DeptCode`, `UnitId`, `DocumentUrl`, `Status` ['Created', 'Closed', 'Cancelled'], `CreatedBy`, `CreatedDate`, `CloseReason`, `ClosedBy`, `ClosedDate`, `CancelReason`, `CancelledBy`, `CancelledDate`) *(หมายเหตุ: ตัดฟิลด์ DocTypeId ออกทั้งหมดตามนโยบาย V3.3.0 ไม่มีการจำแนกประเภทเอกสาร)*
2. **`INTERNAL_DOC_LINK`:** จัดเก็บบันทึกประวัติและรายการลิงก์เอกสารอ้างอิง/SharePoint (`Id`, `RequestId`, `LinkUrl`, `LinkDescription`, `CreatedBy`, `CreatedDate`, `UpdatedBy`, `UpdatedDate`)
3. **`DEPARTMENT_CODE`:** จัดเก็บรหัสตัวย่อฝ่ายสำหรับ Running Number (`Id`, `DeptCode`, `DeptShortTh`, `DeptShortEn`, `RunningScope`, `CurrentSeq`)
*(หมายเหตุ: ปลดระวางและยกเลิกตาราง `INTERNAL_DOC_TYPE` ออกจากสถาปัตยกรรมข้อมูลทั้งหมด)*
...
4. **`AUDIT_LOG`:** บันทึก Audit Trail ย้อนหลัง 10 ปี (`log_id`, `module_name` ['EDR_MEMO', 'CORR_IN', 'CORR_OUT', 'SECURITY'], `action_type` ['CREATE_DOCUMENT', 'CANCEL_DOCUMENT', 'CLOSE_DOCUMENT', 'ADD_DOCUMENT_LINK', 'UPDATE_DOCUMENT_LINK', 'REMOVE_DOCUMENT_LINK', 'COPY_DOC_NO', 'ACCEPT_TASK', 'REJECT_TASK', 'RECALL_TASK', 'DELEGATE_TASK', 'COMPLETE_TASK', 'OTP_REQUEST', 'OTP_VERIFY'], `record_id`, `actor_username`, `dept_code`, `ip_address`, `timestamp`, `details_json`, `reason_text`)
...
|---|---|---|---|---|:---:|
| **RL-EDR-AUTH-01** | **EDR Memo** | Login / เข้าสู่ระบบ | ผู้ใช้งานล็อกอินผ่าน Windows Active Directory สำเร็จในครั้งแรก | **Auto-provisioning ทันที**: ระบบสร้าง User Profile และกำหนดบทบาท `EDR-R01` (User ทั่วไป) อัตโนมัติ ไม่ต้องรอ Admin มาเพิ่มสิทธิ์ | 200 |
| **RL-EDR-ROLE-01** | **EDR Memo** | สิทธิ์ Admin ฝ่าย | ผู้ใช้บทบาท Admin ฝ่าย (`EDR-R02`) เรียกดู/ค้นหา/จัดการเอกสาร | มองเห็นและจัดการงานทั้งหมดภายในฝ่ายตนเอง (Department Scope 100%) แต่**ถูกปิดกั้นการเข้าถึงเมนูตั้งค่า Master Data / System Settings อย่างเด็ดขาด** | 200 / 403 |
| **RL-EDR-ROLE-02** | **EDR Memo** | สิทธิ์ SuperAdmin | ผู้ใช้บทบาท SuperAdmin (`EDR-R03`) เข้าใช้งานระบบ | มีสิทธิ์เต็มทุกประการ เห็นข้อมูลทุกฝ่าย และสามารถจัดการ Master Data / System Settings ได้ทั้งหมด (System Scope) | 200 |
| **RL-CORE-01** | **EDR Memo** | ขอเลขบันทึกภายใน | ไม่กรอกชื่อเรื่องเอกสาร | บล็อกการกดดำเนินการต่อ แสดง Error "กรุณาระบุชื่อเรื่อง" (VR-EDR-01) | 422 |
...
| **RL-CORE-06** | **EDR Memo** | ปิดเลขเอกสาร | กดปิดเลขโดยไม่ระบุเหตุผล หรือระบุสั้นกว่า 5 ตัวอักษร | บล็อกปุ่มยืนยัน แสดง "กรุณาระบุเหตุผลการปิดเลขอย่างน้อย 5 ตัวอักษร" (VR-EDR-CLOSE) | 422 |
| **RL-CORE-08** | **EDR Memo** | ตั้งค่า Master | มีการแก้ไข Format หรือ Pattern ในภายหลัง (เฉพาะ SuperAdmin) | เลขที่ออกไปก่อนหน้ายังคงค่าเดิมตลอดไป ห้ามเปลี่ยนแปลง (Invariant 3) | 200 |
...
| **RL-EDR-CANCEL-01**| **EDR Memo**| ยกเลิกเลขเอกสาร | ขอยกเลิกเลขเอกสารพร้อมระบุเหตุผล $\ge 10$ ตัวอักษร | ปรับสถานะเป็น `Cancelled`, ประทับชื่อผู้ยกเลิก/วันเวลา และบันทึก Audit Log `CANCEL_DOCUMENT` | 200 |
| **RL-EDR-CANCEL-02**| **EDR Memo**| ยกเลิกเลขเอกสาร | ขอยกเลิกเลขเอกสารโดยไม่ระบุเหตุผล หรือสั้นกว่า 10 ตัวอักษร | บล็อกปุ่มยืนยัน แสดงข้อความเตือน "กรุณาระบุเหตุผลการยกเลิกเอกสารอย่างน้อย 10 ตัวอักษร" (VR-EDR-CANCEL) | 422 |
| **RL-EDR-LINK-01** | **EDR Memo** | จัดการลิงก์เอกสาร | เพิ่ม/แก้ไข/ลบ ลิงก์เอกสารอ้างอิงในหน้า Detail | บันทึกการเปลี่ยนแปลง และบันทึก Audit Log (`ADD_DOCUMENT_LINK`, `UPDATE_DOCUMENT_LINK`, `REMOVE_DOCUMENT_LINK`) ทันที | 200/201 |
| **RL-EDR-COPY-01** | **EDR Memo** | One-Click Copy | ผู้ใช้คลิกปุ่ม Copy ข้างเลขที่เอกสาร (Success/Detail/List) | คัดลอกสตริงเลขที่เอกสารลง OS Clipboard พร้อมแสดง Toast "คัดลอกเลขที่เอกสารเรียบร้อยแล้ว" | N/A (UI) |
...
| **VR-EDR-CANCEL**| **EDR Memo**| เหตุผลการยกเลิก | ค่าว่าง หรือน้อยกว่า 10 ตัวอักษร | "กรุณาระบุเหตุผลการยกเลิกเอกสารอย่างน้อย 10 ตัวอักษร" | High | Modal Submit |
| **VR-EDR-LINK** | **EDR Memo** | ลิงก์เอกสารอ้างอิง | ไม่ขึ้นต้นด้วย `https://` หรือเกิน 2,000 ตัวอักษร | "ลิงค์เอกสารต้องขึ้นต้นด้วย https:// และไม่เกิน 2,000 ตัวอักษร" | High | On Submit Link |
...
|---|---|---|:---:|---|---|
| **TS-EDR-AUTH01** | **EDR Memo** | RL-EDR-AUTH-01 | Happy Path | พนักงานล็อกอินผ่าน Windows AD สำเร็จเป็นครั้งแรกโดยยังไม่เคยมีบัญชีในระบบ | Auto-provisioning สร้าง User ทันที พร้อมผูก Role `EDR-R01` (User ทั่วไป) เข้าใช้งานหน้าขอเลขได้ทันทีโดยไม่ต้องรอ Admin |
| **TS-EDR-ROLE01** | **EDR Memo** | RL-EDR-ROLE-01 | Negative | ผู้ใช้บทบาท Admin ฝ่าย (`EDR-R02`) พยายามเข้าถึง URL ตั้งค่า Master Data / System Settings | ระบบบล็อกการเข้าถึง ตอบกลับ HTTP 403 Forbidden และไม่แสดงเมนูตั้งค่าระบบใน Navigation Bar |
| **TS-EDR-ROLE02** | **EDR Memo** | RL-EDR-ROLE-01 | Happy Path | ผู้ใช้บทบาท Admin ฝ่าย (`EDR-R02`) ค้นหารายการเอกสารบันทึกภายใน | แสดงรายการเอกสารทั้งหมดของฝ่ายตนเอง 100% (ทั้งที่ตนเองสร้างและเพื่อนร่วมฝ่ายสร้าง) แต่ไม่เห็นเอกสารของฝ่ายอื่น |
| **TS-EDR-01** | **EDR Memo** | RL-CORE-01 | Negative | ขอเลขบันทึกภายในโดยเว้นว่างชื่อเรื่อง | บล็อกการส่งฟอร์ม แสดง Error "กรุณาระบุชื่อเรื่อง" (VR-EDR-01) |
...
| **TS-EDR-COPY01** | **EDR Memo** | RL-EDR-COPY-01 | Happy Path | ผู้ใช้คลิกปุ่มไอคอน Copy ข้างเลขเอกสาร (Success Dialog / Request Detail / Dashboard) | สตริงเลขที่เอกสารถูกคัดลอกลง Clipboard และแสดง Toast Notification สีเขียว "คัดลอกเลขที่เอกสารเรียบร้อยแล้ว" |
| **TS-EDR-CANCEL01**| **EDR Memo**| RL-EDR-CANCEL-02 | Negative | ผู้ใช้กดยืนยันยกเลิกเอกสารโดยไม่กรอกเหตุผล หรือกรอกข้อความสั้นกว่า 10 ตัวอักษร | บล็อกปุ่มยืนยัน แสดง Error "กรุณาระบุเหตุผลการยกเลิกเอกสารอย่างน้อย 10 ตัวอักษร" (VR-EDR-CANCEL) |
| **TS-EDR-CANCEL02**| **EDR Memo**| RL-EDR-CANCEL-01 | Happy Path | ผู้สร้าง/Admin ฝ่าย/SuperAdmin ระบุเหตุผลยกเลิก $\ge 10$ ตัวอักษรและกดยืนยัน | สถานะเปลี่ยนเป็น `Cancelled`, ล็อกห้ามแก้ไข, และบันทึก Audit Log `CANCEL_DOCUMENT` พร้อมเหตุผล |
| **TS-EDR-LINK01** | **EDR Memo** | RL-EDR-LINK-01 | Happy Path | ผู้ใช้เพิ่ม/แก้ไข/ลบ ลิงก์เอกสารอ้างอิงในหน้า Request Detail | ข้อมูลลิงก์ถูกบันทึกสำเร็จ พร้อมสร้างบันทึก Audit Log (`ADD_DOCUMENT_LINK` / `REMOVE_DOCUMENT_LINK`) |
| **TS-EDR-08** | **EDR Memo** | RL-CORE-06 | Negative | กดปิดเลขเอกสารบันทึกภายในโดยไม่ระบุเหตุผล หรือสั้นกว่า 5 ตัวอักษร | บล็อกปุ่มยืนยัน แสดง "กรุณาระบุเหตุผลการปิดเลขอย่างน้อย 5 ตัวอักษร" |
...
```markdown
GET    /EDR/InternalRequest/Create           - โหลดข้อมูลสังกัด & Preview รูปแบบเลขตามสังกัด (ไม่ต้องเลือกประเภทเอกสาร)
POST   /EDR/InternalRequest/Create           - ขอเลขบันทึกภายในทันที (Atomic Lock, No Approval)
GET    /EDR/InternalRequest/Detail/{id}      - ดึงข้อมูลรายละเอียดคำขอ รายการลิงก์ และประวัติสถานะ
POST   /EDR/InternalRequest/Close            - ปิดเลขเอกสารพร้อมระบุเหตุผล >= 5 ตัวอักษร (Closed) (บันทึก Audit Log)
POST   /EDR/InternalRequest/Cancel           - ยกเลิกเลขเอกสารพร้อมระบุเหตุผล >= 10 ตัวอักษร (Cancelled) (บันทึก Audit Log)
POST   /EDR/InternalRequest/AddLink          - เพิ่มลิงก์เอกสารอ้างอิง/SharePoint พร้อมคำอธิบาย (บันทึก Audit Log)
POST   /EDR/InternalRequest/UpdateLink       - แก้ไขลิงก์เอกสารอ้างอิง (บันทึก Audit Log)
DELETE /EDR/InternalRequest/DeleteLink/{id}  - ลบลิงก์เอกสารอ้างอิง (บันทึก Audit Log)
```
...
```markdown
POST   /api/auth/login                       - ตรวจสอบสิทธิ์ผ่าน LDAP (EDR Memo ทำ Auto-provisioning อัตโนมัติ)
...
|---|---|:---:|---|
| `ERR_AUTH_NOT_PROVISIONED` | Correspondence | 403 | บัญชีมีอยู่ใน LDAP แต่ยังไม่ได้รับมอบหมายสิทธิ์เฉพาะในระบบสารบรรณ (สำหรับ EDR Memo ทำ Auto-provisioning อัตโนมัติ) |
| `ERR_EDR_FORBIDDEN_SETTINGS` | EDR Memo | 403 | ปฏิเสธการเข้าถึงเมนูตั้งค่าระบบ/Master Data (อนุญาตเฉพาะ SuperAdmin เท่านั้น, Admin ฝ่ายและ User ทั่วไปไม่มีสิทธิ์) |
| `ERR_EDR_CANCEL_REASON_REQUIRED` | EDR Memo | 422 | กรุณาระบุเหตุผลการยกเลิกเอกสารอย่างน้อย 10 ตัวอักษร (VR-EDR-CANCEL) |
| `ERR_EDR_INVALID_URL` | EDR Memo | 422 | ลิงก์ SharePoint/เอกสารอ้างอิงไม่ถูกต้อง (ต้องขึ้นต้นด้วย `https://` และไม่เกิน 2,000 ตัวอักษร) |
